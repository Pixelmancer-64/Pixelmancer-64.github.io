...
class Particle {
    ...
    top() {
        ...
        Canvas.ctx.lineTo(this.x * Configs.cellSize + Configs.cellSize, this.y * Configs.cellSize)
    }

    rigth() {
        Canvas.ctx.moveTo(this.x * Configs.cellSize + Configs.cellSize, this.y * Configs.cellSize)
        Canvas.ctx.lineTo(this.x * Configs.cellSize + Configs.cellSize, this.y * Configs.cellSize + Configs.cellSize)
    }

    bottom() {
        Canvas.ctx.moveTo(this.x * Configs.cellSize + Configs.cellSize, this.y * Configs.cellSize + Configs.cellSize)
        Canvas.ctx.lineTo(this.x * Configs.cellSize, this.y * Configs.cellSize + Configs.cellSize)
    }

    left() {
        Canvas.ctx.moveTo(this.x * Configs.cellSize, this.y * Configs.cellSize + Configs.cellSize)
        ...
        if (options.length > 0) {
            ...
            this.closed.forEach((e, index) => {
                if (e.x == chosenOne.x && e.y == chosenOne.y) {
                    ...
                    Canvas.grid[this.y + chosenOne.y][this.x + chosenOne.x].closed[aux].state = false
    ...
    validGrid(x, y) {
        if (x < 0 || x > Canvas.cols - 1 || y < 0 || y > Canvas.rows - 1) return false;
...
class Canvas {
    ...
    constructor() {
        ...
        this.current = Canvas.grid[Math.floor(Math.random() * Canvas.rows)][Math.floor(Math.random() * Canvas.cols)]
        this.gradient()
    ...
    animation() {
        ...
        if (next) {
            ...
            this.current = Canvas.grid[this.current.y + next.y][this.current.x + next.x]

        } else if (Canvas.stack.length) this.current = Canvas.stack.pop();
        else {
            cancelAnimationFrame(this.animationRequest)
            Canvas.grid.forEach(e => e.forEach(cell => cell.draw()))
            Canvas.grid[Math.floor(Math.random() * Canvas.rows)][Math.floor(Math.random() * Canvas.cols)].winner();

            const pixelData = Canvas.ctx.getImageData(0, 0, Canvas.width, Canvas.height)
            ...
            for (let y = 0; y < Canvas.height; y++) {
                ...
                for (let x = 0; x < Canvas.width; x++) {
                    ...
                    const pixel = 'rgba(' + red + ',' + green + ',' + blue + ',' + '1' + ')'
    ...
    gradient() {
        Canvas.gradient = Canvas.ctx.createLinearGradient(0, 0, Canvas.width, Canvas.height);
    ...
    events() {
        ...
        function mouseMove(e) {
            ...
            if(!Configs.mouse.x){
                ...
                if(Math.abs(e.x - Configs.mouse.x) > 10 || Math.abs(e.y - Configs.mouse.y) > 10) {
