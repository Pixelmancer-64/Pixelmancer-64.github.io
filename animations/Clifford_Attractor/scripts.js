...
class Particle{
    ...
    draw(color){
        let xb = canvas.width*this.x/(2.1*(Math.abs(this.c)+1))+canvas.width/2.2;
        let yb = canvas.height*this.y/(2.1*(Math.abs(this.d)+1))+canvas.height/2;
        ctx.beginPath();
        ctx.fillStyle = color;
    ...
    clifford(){
        ...
        this.x = Math.sin(this.a*yPrev) + this.c*Math.cos(this.a*xPrev) 
        this.y = Math.sin(this.b*xPrev) + this.d*Math.cos(this.b*yPrev)
    }
