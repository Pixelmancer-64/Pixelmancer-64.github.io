...
function init() {
  ...
  function get_next_color(r, g, b, x, y) {
    while (true) {
  ...
  function draw(x, y, current) {
    ...
    screen_points[new_x][new_y] = current;
    colors_3d[current.r][current.g][current.b] = true;

    const neighbors = moves.map((option) => {
      return check_valid_filled_colors(x + option.x, y + option.y);
    }).filter(e=>e);

    let t_red =
      neighbors.reduce((total, e) => {
        return total + e.r;
      }, 1) / neighbors.length;
    ...
    // console.log(t_red, t_green, t_blue);
    const [r, g, b] = get_next_color(
      Math.ceil(t_red),
      Math.ceil(t_green),
      Math.ceil(t_blue),
  ...
  function check_valid_filled_colors(x, y) {
    if (x < 0 || x > canvas.width - 1 || y < 0 || y > canvas.height - 1)
      return false;

    return screen_points[x][y];
  }
