...
class Particle {
  ...
  draw(color, x, y) {
    let px = Math.floor(this.map(x, -3, 3, this.width * 0.08, this.width * 0.95));
    let py = Math.floor(this.map(y, -3, 3, this.height * 0.95, this.height * 0.1));
  ...
  animate() {
    for (let i = 0; i < 1000000; i++) this.equations(Math.random());
    this.histogram.forEach((data) => {
      let color = "rgba(223, 223, 223, .8)";
